...
A high-speed file copy tool for Windows, inspired by [TeraCopy](https://teracopy.en.softonic.com/). Built with Rust.
Features a parallel copy engine, hash verification, Windows shell integration, and a space-themed particle UI.
...
| 🪟 Shell Integration | Right-click menu, Send To, Paste |
| 🔁 Move Mode | Copy + delete originals (exponential backoff + jitter retry) |
| 🖥 GUI / CLI | Both interfaces available |
| 🌟 Dot Design UI | Background particles, dot progress bars, star-shaped checkboxes |
...
| 🧩 Plugin Architecture | Bevy-inspired `Plugin` trait + `App` builder for modular features |
